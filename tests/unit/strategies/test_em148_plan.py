"""The pre-declared EM-148 plan is sound before any hours are spent running it: every candidate
...
PLAN = yaml.safe_load(Path("config/curation/plan_em148.yaml").read_text())
...
def test_squeeze_breakout_v1_is_declared_once_with_six_named_candidates() -> None:
    names = [s["name"] for s in PLAN["strategies"]]

    assert names == ["squeeze_breakout_v1"]
...
def test_squeeze_breakout_v1_is_not_enabled() -> None:
    assert "squeeze_breakout_v1" not in [c.name for c in _loader().load_enabled()]
