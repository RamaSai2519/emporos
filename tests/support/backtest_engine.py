...
def engine(
    ...
    if gate is None:
        return BacktestEngine(reader, registry(), FixedTicks(), lambda: source)
    return BacktestEngine(reader, registry(), FixedTicks(), lambda: source, gate)
...
async def run(
    ...
    BuyThenSell.reset()
    return await engine(candles, gate).run(spec(strategy, days, fills))
