...
> {
  ...
  componentDidUpdate(previous: Readonly<typeof this.props>) {
    if (previous.action !== this.props.action && this.state.typed)
      this.setState({ typed: "" });
  }
  private submit = (event: FormEvent) => {
    ...
    if (
      !action ||
      this.props.disabled ||
      (action.confirmation && this.state.typed !== action.confirmation)
    )
      return;
    this.props.onSubmit(action.draft);
    this.props.onClose();
  };
  render() {
    const action = this.props.action;
            ...
            <form onSubmit={this.submit}>
              {action?.confirmation && (
                <label className="field">
                  Type <strong>{action.confirmation}</strong> to confirm
                  <input
                    autoComplete="off"
              ...
              <p className="muted small">
                The worker validates this request. Its outcome will appear in
              ...
              <div className="dialog-actions">
                ...
                <Button
                  ...
                  disabled={
                    this.props.disabled ||
                    (!!action?.confirmation &&
                      action.confirmation !== this.state.typed)
                  }
                >
                  {action?.title}
                </Button>
