...
__all__ = ["ArmResult", "BarSource", "CellScreenRun", "ScanFactory"]
...
class CellScreenRun:
    ...
    def run(
        ...
        dropped = [0] * len(arms)
        for instrument_id in self._audit.instrument_ids:
            ...
            for index, scan in enumerate(scans):
                for trade in scan.scan(instrument_id, series):
                    ...
                    if self._audit.is_quarantined(instrument_id, trade.day):
                        dropped[index] += 1
                        continue
    ...
    def _screen(
        ...
        identity = ScreenIdentity(
            hypothesis, dict(arm), self._audit.universe_label,
