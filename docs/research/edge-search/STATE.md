...
| Cells | 36 TODO, 0 BLOCKED, 12 terminal (all SCREEN_REJECT); D5b (EM-212) gates 4 L2 cells; foundations F1-F4, VAULT and D8 are DONE, so cells without a D-dependency may run |
...
## Iteration 21 (2026-09-25): L2-in-session-results-drift (EM-231)
Track C run by Agent 2 under PROFIT_PLAN §6. **Declared first** (68d9714): the first-public results filing
of each season for a D1 name, published between 09:15 and 14:30 IST (every earlier L2 cell skipped these
1,447 events); anchor = close of the last 5m bar that ended at or before the release; reaction bar = the
first bar starting at or after release + 5 minutes (so a bar that began before the release can never be
the reaction bar; a test pins it, and a mutation of the buffer to zero fails 17 tests); m = reaction close
over anchor; entry at its close if |m| >= 1.5 or 3.0%, continuation or fade, hold to 15:15, Rs 50,000,
D1, Discovery. Prior stated: about 10%. Known issue stated: 114 events at 3.0% cannot reach the 300-trade
floor.

**Result: every arm SCREEN_REJECT, all feasible on the median move.** Events measured per year 2016:43,
2017:154, 2018:143, 2019:108, 2020:99, 2021:130, 2022:85, 2023:143, 2024:136 (skipped: 3,156 outside
the window, 17 with no session bars, 0 without an anchor or a reaction bar). Continuation gross +0.002%
(345 trades, |m| >= 1.5%, 183 long / 162 short) and -0.032% (114 trades, >= 3.0%); fade -0.017% (344) and
+0.001% (113); net -0.23..-0.26%, t -0.76..-1.65; median |move| 1.40-1.99%. The falsification
(continuation gross under 0.25%) holds, so **the in-session results lane is closed, no child**. The
news-driven drift the mechanism predicts is over within the five-minute buffer on these names.
